...
def try_loop_phase_setting(code, setting):
    ...
    for loop in itertools.count():
        for ampnum in range(5):
            phase = setting[ampnum]
            if loop == 0:
                outval = amp[ampnum].run([phase, inval], return_output=True)
            else:
                outval = amp[ampnum].run([inval], return_output=True)
            inval = outval
        if amp[4].returned_on == 'terminate':
            return amp[4].outvalues[-1]
